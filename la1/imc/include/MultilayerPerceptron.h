...
namespace imc
{
	...
	private:
		...
		void generateValidationData(Dataset *trainDataset, Dataset **tD, Dataset **vD);
		double randomDouble(const double &min, const double &max);
		void clearDeltaWeights();

		Dataset *permutateDataset(Dataset *dataset);

		Dataset *datasetFromIndexes(Dataset *original, int *indexes, int size);
