...
private:
    ...
    void weightAdjustment(const int nOfPatterns = 1); //For offline mode, where we average the derivatives
    ...
    void splitDataset(Dataset* dataset, Dataset** train, Dataset** validation);
    Dataset* datasetFromIndexes(Dataset* dataset, int* indexes, int size);
